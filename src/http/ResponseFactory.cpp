...
namespace {
	...
	bool readRegularFile(const std::string& path, std::string& outContent) {
		if (path.empty()) {
			return false;
		}
		struct stat st;
	...
	bool loadConfiguredErrorPage(int code, const ServerConfig& cfg, std::string& outBody) {
	...
	std::string makeBuiltinErrorPage(int code) {
